...
class TakeCareOfCatViewModel: ObservableObject {
    @Published var allCareCatList:[String] = []
    @Published var temCatId:[String?] = []
    @Published var careCatList:[UserInfo]?
    ...
    func fileterCat() {
        if let interestCatList = careCatList {
            ...
            for cat in interestCatList {
                ...
                temCatId.append(contentsOf: cat.care_cat_ids)
            }
